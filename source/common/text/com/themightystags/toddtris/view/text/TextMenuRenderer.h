...
namespace com {
	namespace themightystags {
		namespace toddtris {
			namespace view {
				namespace text {
					class TextMenuRenderer: public MenuRenderer {
					public:
						TextMenuRenderer();
						virtual ~TextMenuRenderer();
						void render(const vector<string> &options, 
								const vector<bool> &selected, bool full);
						static TextMenuRenderer &getInstance();
						
					private:
						static TextMenuRenderer *INSTANCE;
						static bool initialised;
						vector<string> lastOptions;
						vector<bool> lastSelected;
					};
				}
			}
		}
